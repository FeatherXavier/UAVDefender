...
using YoloBackend.Scorer.Extensions;
...
public class YoloScorer<T> : IDisposable where T : YoloModel
{
    ...
    SessionOptions gpuSessionOptions;
    ...
    private DenseTensor<float>[] Inference(Image<Rgba32> image)
    {
        ...
        {
            output.Add(result.FirstOrDefault(x => x.Name == item).Value as DenseTensor<float>);
        };
    ...
    /// </summary>
    public YoloScorer(string weights, string BackendType, SessionOptions opts = null) : this()
    {

        gpuSessionOptions = new();
        try
        {
            switch (BackendType)
            {
                case "CPU":
                    gpuSessionOptions.AppendExecutionProvider_CPU();
                    backendType = "CPU";
                    break;
                case "CUDA":
                    gpuSessionOptions.AppendExecutionProvider_CUDA();
                    backendType = "CUDA";
                    break;
                case "ROCm(当前系统不支持)":
                    throw new NotImplementedException();
                case "DirectML(建议)":
                    gpuSessionOptions.AppendExecutionProvider_DML();
        ...
        {
            Console.WriteLine("ROCm is not supported on Windows.\nThis is the problem of AMD, NOT ME!!!");
        }

        _inferenceSession = new InferenceSession(File.ReadAllBytes(weights), gpuSessionOptions);
    }
